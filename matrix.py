...
from exeption.matr_exeption import (
...
class Matrix:
    def __init__(
        self, rows: int, cols: int, data: list[list[int | float] | int | float] = None
    ) -> None:
        self.__rows = rows
        self.__cols = cols
        ...
        if data is None:
            ...
            if isinstance(data, list) and (
                self.rows != len(data) or self.cols != len(data[0])
            ):
                raise NotValidDataDimensionExeption(rows, cols, len(data), len(data[0]))

            self._data_type = (
                "d"
                if type(data) is int or type(data[0]) is int or type(data[0][0]) is int
                else "f"
            )
    ...
    def __str__(self) -> str:
        ...
        for row in self._data:
            l = [
                f"{val:>6.2f}" if self._data_type == "f" else f"{val:>4d}"
                for val in row
            ]
            res.append(", ".join(l))
        return "\n".join(res)
    ...
    def fill_random_int(self, start: int = -10, stop: int = 10):
        ...
        self._data_type = "d"
        self._data = [
            [random.randrange(start, stop + 1) for _ in range(self.cols)]
    ...
    def fill_random_float(self, start: float = -10, stop: float = 10):
        ...
        self._data_type = "f"
        self._data = [
            [random.uniform(start, stop) for _ in range(self.cols)]
            for _ in range(self.rows)
        ]
    ...
    def __validate(self, other: Self, add: bool = True) -> None:
        ...
        if add:
            if self.rows != other.rows or self.cols != other.cols:
                raise AddOperMatrixExeption(
                    self.rows, self.cols, other.rows, other.cols
                )
        else:
            if self.cols != other.rows:
                raise MulOperMatrixExeption(
                    self.rows, self.cols, other.rows, other.cols
                )
...
def test_matrix():
    ...
    ml = Matrix(2, 3, [[1, 3, -2], [-1, 1, 0]])
    print(f"ml = \n{ml}")
    mr = Matrix(2, 1, [[-1], [1]])
    print(f"mr = \n{mr}")
    try:
        m_mul = ml * mr
    except MulOperMatrixExeption as e:
